...
cols_to_remove <- c("biceps_cir", "forearm_cir", "wrist_cir", "head_cir", "Trial", "Initial", "Peak", "Min", "MinPeak", "Subject", "studyid")
...
```{r}
mod <- lm(InitialPeak ~ .-leg_l_l - leg_l_r - BMI - waist_cir - hip_cir - L_thigh_cir -thigh_cir - height_m - weight_kg - W.H.ratio - Hip.Index - WA_D -HIP_B  - BS - SH_D - CH_D  -Class, data = numeric_df)
car::vif(mod)
```
...
```{r}
# Set desired order of Task
df$Task <- factor(df$Task, levels = c("PRF", "FF", "OCMB", "OCHB", "OCMA", "OCHA"))
...
ggplot(df, aes(x = Task, y = MinPeak, fill = GroupF)) +
  geom_boxplot(position = position_dodge(width = 0.8),
               width    = 0.6) +
  geom_jitter(aes(colour = GroupF),
              position = position_jitterdodge(
  ...
  scale_colour_brewer(palette = "Set2") +
  labs(
    title  = "PeakMin Knee Angles by Task and Body-Mass Group",
    x      = "Task",
    y      = "PeakMin Knee Degrees",
    fill   = "Group",
    colour = "Group"
  ) +
  theme_minimal(base_size = 14)
...
```{r}
#df <- mutate(df, Obstacle_new = ifelse(Obstacle == 0, 0, 1))
#df <- subset(df, Task %in% c('PRF', 'FF', 'OCMB', 'OCHB', 'OCMA', 'OCHA'))
```
...
```{r}
df <- read.csv('BodyShape.csv')
df_prf <- subset(df, Task == 'PRF')
df_ff <- subset(df, Task == 'FF')
df_ocmb <- subset(df, Task == 'OCMB')
df_ochb <- subset(df, Task == 'OCHB')
df_ocma <- subset(df, Task == 'OCMA')
df_ocha <- subset(df, Task == 'OCHA')
head(df_ocma)
             # Should show all original columns

# --- Create a correlation matrix for the selected tasks ---
```
...
```{r, echo=TRUE}
library(lmerTest)
m1 <- lmer(Initial ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ocma)
summary(m1)
```
...
```{r, echo=TRUE}
m2 <- lmer(Peak ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ocma)
summary(m2)
```
...
m3 <- lmer(InitialPeak ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ocma)
summary(m3)
```
...
```{r}
m4 <- lmer(InitialPeak ~ Group+Sex+Race+leg_l+Speed+HIP_D+L_thigh_cir+shank_cir+ASIS+(1|Subject), 
           data=df_ocma)
summary(m4)
```

```{r}
m5 <- lmer(Initial ~ Group+leg_l+Speed+HIP_D+L_thigh_cir+shank_cir+Sex+Race+ASIS+(1|Subject), 
           data=df_ocma)
summary(m5)
```
```
...
```
```{r}
m6 <- lmer(Peak ~ Group+leg_l+Speed+HIP_D+L_thigh_cir+shank_cir+Sex+Race+ASIS+(1|Subject), 
           data=df_ocma)
summary(m6)
```

```{r}
m7 <- lmer(MinPeak ~ Group+leg_l+Speed+HIP_D+L_thigh_cir+shank_cir+Sex+Race+ASIS+(1|Subject), 
           data=df_ocma)
summary(m7)
```
...
```{r}
fixed_formula <- InitialPeak ~ Group +Task+ age + Sex + Race + leg_l + 
...
# --- Create GroupF and make factors ---
df <- df %>%
  mutate(
    GroupF = factor(ifelse(Group == 0, "Normal", "Obese"), levels = c("Normal", "Obese")),
    Task = factor(Task)
  )

# Optional: Check your levels
# table(df$GroupF)
# table(df$Task)

# --- Filter NA values before running t-tests ---
df_filtered <- df %>%
  filter(!is.na(InitialPeak), !is.na(GroupF), !is.na(Task))

# --- Perform t-tests by Task ---
task_ttests <- df_filtered %>%
  group_by(Task) %>%
