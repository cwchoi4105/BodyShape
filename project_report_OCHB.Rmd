...
```{r, echo=TRUE}
library(lmerTest)
m1 <- lmer(Initial ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ochb)
summary(m1)
```
...
```{r, echo=TRUE}
m2 <- lmer(Peak ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ochb)
summary(m2)
```
...
m3 <- lmer(InitialPeak ~ Group+Sex+Race+leg_l+Speed+(1|Subject), data=df_ochb)
summary(m3)
```
